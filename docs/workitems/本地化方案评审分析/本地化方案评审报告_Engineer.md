...
### 决策 2: 用户自定义 LLM Provider

| 项 | 内容 |
|----|------|
| **决策** | 用户自行配置 API Key 和 Provider，不限定特定服务商 |
| **实现** | 通过 `hermes-config.yaml` 的 `custom` provider + 用户提供的 `base_url` 和 `api_key` |
| **优势** | 用户可选择 OpenAI、DeepSeek、智谱、腾讯云等任意兼容端点 |
| **配置方式** | 用户在 `.env` 或 Web UI 设置页面配置 |
